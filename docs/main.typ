...
= Вывод
В ходе лабораторной работы была доработана информационная система:
- Добавлена реализация импортов объектов через JSON файл\
- Добавлены ограничения уникальности предметной области\
- Критические операции выполняются в явных транзакциях (протестировано через Jmeter)